...
def train(train_loader, encoder, decoder, criterion, encoder_optimizer, decoder_optimizer, epoch):
    ...
    for i, (imgs, caps, caplens) in enumerate(train_loader):
        ...
        imgs = encoder(imgs)
        # breakpoint()
        scores, caps_sorted, decode_lengths, alphas, sort_ind = decoder(imgs, caps.to(torch.int), caplens.to(torch.int))
        ...
        s_pack = pack_padded_sequence(scores, decode_lengths, batch_first=True)
        scores = s_pack.data
        t_pack = pack_padded_sequence(targets, decode_lengths, batch_first=True)
        targets = t_pack.data

        # Calculate loss
        loss = criterion(scores, targets.to(torch.long))
